...
function updateGrid () {
    ...
    for (let row = 0; row <= 11; row++) {
        ...
        for (let column = 0; column <= 15; column++) {
            currentCallAliveorDead = grid[row][column]
            currentCellNeighbors = countNeighbors(row, column)
            if (currentCallAliveorDead == 1 && currentCellNeighbors < 2) {
                nextGrid[row].push(0)
            } else if (currentCallAliveorDead == 1 && currentCellNeighbors > 3) {
                nextGrid[row].push(0)
            } else if (currentCallAliveorDead == 0 && currentCellNeighbors == 3) {
                nextGrid[row].push(1)
            } else {
                nextGrid[row].push(currentCallAliveorDead)
            }
        }
    }
    grid = nextGrid
